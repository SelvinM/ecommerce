...
const Mutation: Required<MutationResolvers> = {
	...
	placeOrder: async (
		...
		if (payAtDelivery) {
			if (
				...
				const orderDoc = new (Order as Model<OrderDocument>)({
					...
					user: user.id,
					paymentMethod: {
						name: PaymentMethod.PayAtDelivery,
					},
					shippingAddress,
					shipments: [
						{
							...
							orderItems: [
								{
									...
									title:
										cartFullTranslations.instaCheckout.product.listing
											.longTitle,
				...
				try {
					...
					await sendOrderInvoiceEmail({
						...
						addressLine2:
							order.shippingAddress.address.addressLine2 || undefined,
					});
					session.endSession();
					return { order, cart };
				} catch (error) {
		...
		return { message: "Debes aceptar pagar con efectivo para hacer la compra" };
